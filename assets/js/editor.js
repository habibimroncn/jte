...
var askContentToServer = async.Task(function (reject, resolve) {
  $.ajax({
    url: ajaxurl,
    method: 'GET',
...
function sendDiffPatchContent(patch) {
  return async.Task(function (reject, resolve) {
    $.ajax({
      url: ajaxurl,
      method: 'POST',
      success: resolve,
...
function diffContentWithServer(content) {
  return async.Task(function (_, resolve) {
    var dmp = new diffmatch.diff_match_patch();
    var diff = dmp.diff_main(syncendContent, content, true);
    if (diff.length > 2) {
      dmp.diff_cleanupSemantic(diff);
    }
    var patch_list = dmp.patch_make(syncendContent, content, diff);
    resolve(dmp.patch_toText(patch_list));
  }).chain(function (r) {
    return async.forkedTask(broadcastDiffPatchContent(r)).chain(function () {
      return async.Task.of(r);
    });
  });
...
function diffTitleWithServer(title) {
  return async.Task(function (_, resolve) {
    var dmp = new diffmatch.diff_match_patch();
    var diff = dmp.diff_main(syncendTitle, title, true);
    if (diff.length > 2) {
      dmp.diff_cleanupSemantic(diff);
    }
    var patch_list = dmp.patch_make(syncendTitle, title, diff);
    resolve(dmp.patch_toText(patch_list));
  }).chain(function (r) {
    return async.forkedTask(broadcastDiffPatchTitle(r)).chain(function () {
      return async.Task.of(r);
    });
  });
  // return askContentToServer.map(res => {
...
var sendSyncPost = debounce(function (content) {
  return diffContentWithServer(content).fork(noop, noop);
}, 700);
var sendSyncTitle = debounce(function (title) {
...
function main() {
  ...
  askContentToServer.map(function (res) {
    // title
    var t = res.title;
...
},{"./diffmatch":1,"./task":3}],3:[function(require,module,exports){
'use strict';
...
function generatorStep(n, d, last) {
  ...
  var _next = next(last.value),
      done = _next.done,
      value = _next.value;

  return done ? value.map(d) : value.map(function (x) {
    return n({ value: x, next: next });
  });
}
...
function forkTask(error, success, computation) {
  var open = true;
  var canceller = computation(function (err) {
    if (open) {
      open = false;
      error(err);
    }
...
Task.prototype.map = function (func) {
  ...
  return new Task(function (error, success) {
    return _this.fork(error, function (v) {
...
Task.prototype.chain = function (func) {
  ...
  return new Task(function (error, success) {
    var canceller = noop;
    var selfCanceller = _this3.fork(error, function (v) {
      var task = func(v);
      canceller = task.fork(error, success);
    });
    return canceller ? canceller : (canceller = selfCancel, function () {
      return canceller();
    });
  });
};
...
Task.prototype.alt = function (other) {
  ...
  return new Task(function (error, success) {
    var canceller2 = noop;
    var canceller1 = _this4.fork(function (_) {
      canceller2 = other.fork(error, success);
    }, success);
    return canceller2 ? canceller2 : (canceller2 = canceller1, function () {
      return canceller1();
    });
  });
};
...
Task.prototype.sequential = function () {
  ...
  return new Task(function (error, success) {
    return _this5.fork(error, success);
  });
};
...
TaskPar.prototype.map = function (func) {
  ...
  return new TaskPar(function (error, success) {
    return _this6.fork(error, function (v) {
      return success(func(v));
    });
  });
};
...
TaskPar.prototype.ap = function (other) {
  ...
  return new TaskPar(function (error, success) {
    ...
    var otherOk = 0;
    var ko = 0;
    var guardReject = function guardReject(x) {
      return ko || (ko = 1, error(x));
    };
    var canceller = _this7.fork(guardReject, function (v) {
      if (!otherOk) return void (thisOk = 1, value = v);
      return success(func(v));
    });
    var canceller1 = other.fork(guardReject, function (f) {
...
TaskPar.prototype.alt = function (other) {
  ...
  return new TaskPar(function (error, success) {
    ...
    var failed = 0;
    var va = void 0;
    var guardReject = function guardReject(v) {
      if (failed === 1) {
        error(va);
      } else {
        va = v;
    ...
    var guardResolve = function guardResolve(v) {
      if (settled) return;
      settled = true;
      success(v);
    };
    var cancellers = [_this8, other].map(function (x) {
      return x.fork(guardReject, guardResolve);
    });
    return function () {
...
TaskPar.prototype.parallel = function () {
  ...
  return new TaskPar(function (error, success) {
    return _this9.fork(error, success);
  });
};
...
function patchFantasyLandMethod(constructor) {
  // Functor
  var fantasyM = ['map', 'ap', 'chain', 'alt'];
  var fantasyS = ['of', 'chainRec', 'zero'];
  fantasyM.forEach(function (method) {
    if (typeof constructor.prototype[method] === 'function') {
      constructor.prototype['fantasy-land/' + method] = constructor.prototype[method];
    }
  });
  // static method
  fantasyS.forEach(function (method) {
    // applicative
...
function forkedTask(forkable) {
  return new Task(function (_, success) {
    var canceller = forkable.fork(voidF, voidF);
    success(canceller);
    return voidF;
  });
}
...
function attempt(f, g) {
  return function (forkable) {
    return new Task(function (_, success) {
      return forkable.fork(function (err) {
